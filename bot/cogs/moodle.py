...
class Moodle(Cog):
    ...
    async def get(self, ctx, option=""):
        ...
        if str(ctx.channel.id) in allowed_channels:
            ...
            if option == "assignments":
                database = await self.client.pg_con.fetch("SELECT * FROM moodle_events WHERE subject_type = $1 AND guild_id = $2", "Tarefa para entregar via Moodle", guild_id)
            elif option == "classes":
                database = await self.client.pg_con.fetch("SELECT * FROM moodle_events WHERE subject_type = $1 AND guild_id = $2", "Aula ao vivo - BigBlueButton", guild_id)
            elif option == "events":
                database = await self.client.pg_con.fetch("SELECT * FROM moodle_events WHERE guild_id = $1", guild_id)
    ...
    async def check(self, ctx):
        ...
        if str(ctx.channel.id) in allowed_channels:
            ...
            tokens_data = await self.client.pg_con.fetch("SELECT token FROM moodle_profile WHERE discord_id = $1 AND guild_id = $2", user_id, guild_id)
    ...
    async def getToken(self, ctx):
        ...
        if str(ctx.channel.id) in allowed_channels:
            ...
            user_data = await self.client.pg_con.fetch("SELECT token FROM moodle_profile WHERE discord_id = $1 AND guild_id = $2", user_id, guild_id)
            ...
            if not user_data:
                ...
                if answer.content.lower() == "yes":
                    ...
                    Token.create(params, username=username.content, password=password.content)
